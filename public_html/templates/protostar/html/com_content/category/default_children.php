...
<?php if (count($this->children[$this->category->id]) > 0) : ?>
	...
	<?php foreach ($this->children[$this->category->id] as $id => $child) : ?>
		...
		<?php if (in_array($child->access, $groups)) : ?>
  		<?php
      $child->numitems = MulticatsContentHelperRoute::getNumItems($child->id);  //Multicats
  		if ($this->params->get('show_empty_categories') || $child->numitems > 0) :
  		...
  		<div<?php echo $class; ?>>
              ...
              <?php echo $child->numitems; // Multicats ?> 
  					</span>
  				<?php endif; ?>
  				<a href="<?php echo JRoute::_(MulticatsContentHelperRoute::getCategoryRoute($child->id)); //Multicats ?>">
  			...
  			<?php else : ?>
  			<h3 class="page-header item-title"><a href="<?php echo JRoute::_(MulticatsContentHelperRoute::getCategoryRoute($child->id)); //Multicats ?>">
  				<?php echo $this->escape($child->title); ?></a>
  				<?php if ( $this->params->get('show_cat_num_articles', 1)) : ?>
  					<span class="badge badge-info tip hasTooltip" title="<?php echo JHtml::_('tooltipText', 'COM_CONTENT_NUM_ITEMS_TIP'); ?>">
  						<?php //echo $child->getNumItems(true); ?>
