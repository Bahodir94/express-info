        ...
        <div class="block-content">
            <table class="table table-responsive table-bordered table-striped table-vcenter js-dataTable-full">
                ...
                @if($category->hasSites())
                    @foreach($category->sites as $site_list)
                        <tr>
                            <td class="text-center">{{ $site_list->id }}</td>
                            ...
                            <td class="text-center d-flex align-items-center justify-content-center">
                                ...
                                <select name="position" class="position" data-id="{{ $site_list->id }}">
                                    @for($i = 0; $i <= count($category->sites); $i++)
                                        <option value="{{ $i }}" @if($site_list->position == $i) selected @endif>{{ $i }}</option>
                                    @endfor
                                </select>
                            </td>
                        </tr>
...
@section('js')
    ...
    <script>
        ...
        $('.position').change(function () {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route('admin.cgusites.change.position') }}',
                dataType: 'json',
                data: formData,
                processData: false,
                contentType: false,
